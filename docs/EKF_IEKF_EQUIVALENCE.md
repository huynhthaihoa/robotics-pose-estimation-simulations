...
- **Motion model**: `T_pred = T_prev (+) Exp(twist * dt)` — constant
  body-frame twist. This is *group-affine*, so the predict step's linearization
  is exact, not first-order — true for both filters, not part of the
...
## What actually differs between them: speed, not accuracy
...
## When would EKF and IEKF actually diverge?
